...
public class Member extends JFrame implements ActionListener{
	...
	public void checkValue() {
		...
		//String str = t_pass.getText(); //deprecated 되었으므로, 사용하지 않아야한다
		//System.out.println("당신이 입력한 비밀번호는"+str); //잘 나오지만, 쓰면 안될것임
		//해결책??
		//char[] pass = t_pass.getPassword(); //char 배열로 데이터 받은후, 우리가 원하는 형태인 String으로 다시 변환
		//char배열을 String으로 변환해서 쓰자
		...
		if(t_id.getText().length() ==0) {
			...
			JOptionPane.showMessageDialog(this, "적어도 1개는 입력해주세요");
		}else{
			//오라클 DB입력
		}
	}
